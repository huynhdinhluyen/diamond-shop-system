...
const PaymentPage = () => {
  ...
  const handleOrder = async (transactionStatus = "INCOMPLETE") => {
    if (
      paymentMethod === "" ||
      paymentMethod === "Chọn phương thức thanh toán của bạn"
    ) {
  ...
  return (
    <PayPalScriptProvider
      options={{
        "client-id":
          "AX4x0xZ5zRYc99ovorXk5ouwFOtobLBMYgGR6JaFiB31vvEH2Z2TAk-qUouX_H9y6ligOQpKYPkZzIVH",
      }}
    >
      <div className="container mt-10 bg-gray-50 p-5 rounded-xl grid grid-cols-12 gap-4">
        <div className="col-span-8 bg-white p-3">
            ...
            <tbody>
              {products.map((product, index) => (
                <tr key={index} className="hover:bg-gray-100">
                  ...
                  <td className="py-3 px-4 border-b border-gray-300 text-[15px]">
                    {["Dây chuyền", "Nhẫn", "Vòng tay"].includes(
                      product.category.name
                    )
                      ? product.category.name === "Dây chuyền"
                        ? product.size + "cm"
                        : product.size + "mm"
                      : "N/A"}
                  </td>
                  ...
                  <td className="py-3 px-4 border-b border-gray-300">
                    <Price price={product.unitPrice * product.quantity} />
                  </td>
                </tr>
              ))}
            </tbody>
          ...
          >
            ...
            <option value="Trả tiền mặt sau khi nhận hàng">
              Trả tiền mặt sau khi nhận hàng
            </option>
          </select>
        ...
        <div className="col-span-4 p-3 bg-white">
          ...
          <div className="mt-4">
            ...
            <div className="flex justify-between">
              <span>Giảm giá:</span>
              <span>
                <Price price={order.discountPrice} />
              </span>
            </div>
            <div className="flex justify-between font-semibold">
              <span>Tổng thanh toán:</span>
              <span>
                <Price price={totalPaymentVND} />
              </span>
            </div>
          </div>
          <button
            onClick={() => handleOrder()}
            className="btn btn-accent btn-lg my-3 mx-auto w-[75%]"
          >
            Đặt hàng
          </button>
        </div>
